    ...
    gray_scale = tf.reshape(
        tf.reduce_mean(
            tf.image.resize_images(
                imgs_norm, size=[224, 224], method=tf.image.ResizeMethod.BILINEAR
            ),
            axis=3,
        ),
        [-1, 224, 224, 1],
...
def gray_scaling(imgs):
    return tf.reshape(
        tf.reduce_mean(
            tf.image.resize(
                imgs, size=[224, 224], method=tf.image.ResizeMethod.BILINEAR
            ),
            axis=3,
        ),
        [-1, 224, 224, 1],
...
def tf_augment_images(data):
        ...
        [
            ...
            gray_scaling(
                tf.contrib.image.rotate(
                    data,
                    tf.random.uniform([1], minval=-0.5235988, maxval=0.5235988),
    ...
    flip_left_right = tf.concat(
        [random_rotations, gray_scaling(tf.image.flip_left_right(data))], 0
    )
